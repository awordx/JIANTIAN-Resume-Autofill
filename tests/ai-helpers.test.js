...
test("AI prompt requests semantic, numbered education-record matching", async () => {
  let request;
  const worker = loadWorker((body) => { request = body; });
  await worker.handleAiFill({
    aiConfig: AI_CONFIG,
    formFields: [{ fieldId: "school", label: "院校", group: "教育经历1", inputType: "text", options: [] }],
    resumeFields: [{ group: "教育经历1", key: "学校", value: "甲大学" }]
  });
  const prompt = request.messages.map((message) => message.content).join("\n");
  assert.match(prompt, /学校=院校=毕业院校/);
  assert.match(prompt, /教育经历1只能使用教育经历1的数据/);
});
...
test("findSelectOptionIndex: negated options never stand in for the positive one, and back", () => {
  ...
  assert.equal(helpers.findSelectOptionIndex(options(["全国普通高等院校全日制", "全国普通高等院校非全日制"]), "全日制"), 0);
  assert.equal(helpers.findSelectOptionIndex(options(["全国普通高等院校全日制", "全国普通高等院校非全日制"]), "非全日制"), 1);
  assert.equal(helpers.findSelectOptionIndex(options(["全国普通高等院校全日制", "全国普通高等院校非全日制", "高中全日制", "全日制中专", "非全日制中专"]), "全日制"), 0);
