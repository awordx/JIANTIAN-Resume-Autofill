...
// 「备份里没有模板」。
test("我的信息被剔空之后不写进备份，只导出剩下的", async () => {
  const popup = loadPopup();
  const saved = captureDownloads(popup);
  ...
  assert.match(popup.lastStatusFrom("backup-status"), /1 份我的信息因此没有内容，未写入/);
});
...
test("空插件导入备份就是直接恢复，不用再问", async () => {
  ...
  assert.match(fresh.lastStatusFrom("backup-status"), /已恢复 2 份我的信息/);
});
...
test("导出的 Excel 能被导入原样解析回来", async () => {
  ...
  assert.equal(saved.length, 1);
  assert.equal(saved[0].fileName, "我的简历.xlsx", "文件名要能直接再导入回同一个名字");
...
test("名字里的非法字符不会进文件名", async () => {
  const popup = loadPopup();
  const api = backupApi(popup);

  assert.equal(api.templateExportFileName("研发/后端:2026?"), "研发后端2026.xlsx");
  assert.equal(api.templateExportFileName("   "), "我的信息.xlsx");
});
