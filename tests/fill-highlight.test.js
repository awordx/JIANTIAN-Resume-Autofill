...
test("a nonempty input waits for add or replace, while a selected chip is removed directly", async () => {
  ...
  // chip 写入现在和 AI 填写共用同一套反馈：每次成功写入会留下一个 2800ms 的高亮清理定时器，
  // 除此之外不该有别的（尤其是状态提示类）定时器残留。
  assert.deepEqual([...new Set(timers.map((timer) => timer.delay))], [2800]);
});
...
test("统一的填充反馈：成功流光、等待呼吸、失败抖动三类样式都在注入的样式表里", () => {
  const { helpers, styleElements } = loadHighlightHelpers();
  helpers.injectFieldHighlightStyles();
  const css = styleElements[0].textContent;

  assert.match(css, /\.resume-pro__field-highlight/);
  assert.match(css, /\.resume-pro__field-waiting/);
  assert.match(css, /\.resume-pro__field-error/);
  assert.match(css, /@keyframes resume-pro-field-flash/);
  assert.match(css, /@keyframes resume-pro-field-breathe/);
  assert.match(css, /@keyframes resume-pro-field-shake/);
});

test("写不进去的字段抖一下（失败反馈），不再是没反应", () => {
  const { helpers, timers, HTMLElement, HTMLInputElement } = loadHighlightHelpers();
  const input = new HTMLInputElement();
  input.value = "写进去的值";
  input.rect = { top: 10, left: 10, bottom: 40, right: 200 };

  helpers.shakeField({ kind: "element", element: input });

  assert.equal(input.classList.contains("resume-pro__field-error"), true);
  const cleanup = timers.find((timer) => timer.delay === 900);
  assert.ok(cleanup, "失败反馈也有清理定时器");
  cleanup.callback();
  assert.equal(input.classList.contains("resume-pro__field-error"), false);
});
