(function attachResumeProAIHelpers(globalScope) {
  ...
  function findSelectOptionIndex(options, value) {
    ...
    // 搜索“全日制”时可能同时出现“全国普通高等院校全日制”、
    // “高中全日制”“全日制中专”等结果。优先选以目标结尾的正式选项，
    // 同一优先级保留页面顺序；仍无法区分时才放弃猜测。
    const suffixCandidates = candidates.filter((optionIndex) =>
      normalizeText(optionText(list[optionIndex])).endsWith(normalizedTarget)
    );
    if (!suffixCandidates.length) return -1;
    const longestSuffixLength = Math.max(...suffixCandidates.map((optionIndex) => normalizeText(optionText(list[optionIndex])).length));
    const longestSuffix = suffixCandidates.filter((optionIndex) => normalizeText(optionText(list[optionIndex])).length === longestSuffixLength);
    return longestSuffix.length === 1 ? longestSuffix[0] : -1;
  }
