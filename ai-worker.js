...
const AI_SYSTEM_PROMPT = [
  ...
  "8. 匹配考虑同义词和同一语义：手机=电话=联系方式=mobile=phone；学历=最高学历=培养层次；学校=院校=毕业院校；开始时间=入学时间；结束时间=毕业时间；成绩=GPA/绩点；学院=院系；学习形式=培养方式。语义明确时应填写，不因字面不同跳过",
  ...
  "11. 重复教育经历按 group 中的编号一一对应：教育经历1只能使用教育经历1的数据，教育经历2只能使用教育经历2的数据；每段内所有语义明确的字段都尽量返回",
  "12. 简历字段列表里 source=template 的是本次投递用的简历（真实数据），source=profile 的是「我的信息」；同一语义字段两处都有时以 template 为准，profile 只用来补 template 没有的字段"
].join("\n");
...
async function handleAiFill(message, controller = new AbortController()) {
  ...
  if (!resumeFields.length) {
    return { success: false, error: "当前这份我的信息还没有可用字段。" };
  }
...
function buildUserPrompt(formFields, resumeFields) {
  return [
    ...
    "",
    "填写原则：按字段语义而不是字面完全一致匹配；教育经历按 group 编号逐段对应。基本信息优先匹配基本信息分组；教育背景不要填进邮箱、电话、出生日期、籍贯等基础字段；只有语义确实不明或值不适配网页控件时才留空。简历字段里 source=template（本次投递简历）的值优先于 source=profile（我的信息）。"
  ].join("\n");
}
